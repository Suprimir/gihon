...
pub fn add_file(app_handle: tauri::AppHandle, source_path: String) -> Result<(), AppError> {
    ...
    if !Path::new(&source_path).exists() {
        warn!("⚠️ File does not exist: {}", source_path);
    ...
    {
        warn!("⚠️ Unsupported file format: {}", source_path);
...
pub fn get_cover_image(
    ...
    info!("🖼️ Getting cover image for: {}", cbz_path);
...
pub fn load_image_by_index(
    ...
    info!("🖼️ Loading image index {} from: {}", image_index, cbz_path);
...
pub fn edit_metadata_file(
    ...
    info!("✏️ Editing metadata for: {}", cbz_path);
